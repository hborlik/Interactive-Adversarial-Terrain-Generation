...
void ShowFileDialog(bool* open, float scale, std::string& buffer, FileDialogType type) {
    static int fileDialogFileSelectIndex = 0;
    static int fileDialogFolderSelectIndex = 0;
    static std::string fileDialogCurrentPath = current_path();
    ...
    if (open) {
        ...
        std::string selectedFilePath = path{fileDialogCurrentPath} / path{fileDialogCurrentFolder.size() > 0 ? fileDialogCurrentFolder : fileDialogCurrentFile};
        ImGui::PushItemWidth(ImGui::GetWindowWidth());
        ImGui::InputText("", selectedFilePath.data(), selectedFilePath.size());
        ...
        if (ImGui::Button("Choose")) {
            if (type == FileDialogType::SelectFolder) {
                if (fileDialogCurrentFolder == "") {
                    ...
                    buffer = path{fileDialogCurrentPath} / path{fileDialogCurrentFolder};
                    fileDialogFileSelectIndex = 0;
                    fileDialogFolderSelectIndex = 0;
                    fileDialogCurrentFile = "";
                    *open = false;
